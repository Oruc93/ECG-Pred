...
def train_MS(total_epochs=250, 
    ...
    with mlflow.start_run():  # separate run for each NN size and configuration
        ...
        # mapping output sets to latent space
        y_l = encoder.predict(y)
        y_test_l = encoder.predict(y_test)
        """print("y_latent", y_latent)
        print("Shape of y_latent", np.shape(y_latent))
        print("Type of y_latent", type(y_latent))
        exit()"""
        ...
        model.compile(loss= 'MSE',# tl.my_loss_fn, # Loss-Funktion
                    optimizer='Adam',
                    metrics='MAE')
